...
# Author name
st.markdown('<div class="author">By Your Name</div>', unsafe_allow_html=True)
...
st.markdown(
    ...
    <div class="welcome-message">
        ...
        <p>LapValue Predictor offers you a quick and accurate estimation of your laptop's value based on its specifications. Input the details of your laptop and find out its market value instantly!</p>
        <p>Simply enter your laptop's specifications, hit the "Predict Price" button, and let LapValue Predictor provide you with an estimated price.</p>
    </div>
    ''', 
    unsafe_allow_html=True
)
...
st.markdown(
    '''
    <h2 class="stSubheader">How to use LapValue Predictor:</h2>
    <ul class="stMarkdown">
        <li>Select the brand of your laptop.</li>
        <li>Choose the type of your laptop.</li>
        <li>Enter the RAM size.</li>
        <li>Provide the weight of your laptop.</li>
        <li>Select whether it has a touchscreen or not.</li>
        <li>Choose the screen type (IPS or not).</li>
        <li>Enter the screen size and resolution.</li>
        <li>Select the CPU, HDD, SSD, GPU, and OS details.</li>
        <li>Click on "Predict Price" to get the estimated price.</li>
    </ul>
    ''', 
    unsafe_allow_html=True
)

# Define your selectbox to choose the laptop details
st.markdown('<label class="selectbox-label">Brand</label>', unsafe_allow_html=True)
company = st.selectbox('', df['Company'].unique())

st.markdown('<label class="selectbox-label">Type</label>', unsafe_allow_html=True)
type = st.selectbox('', df['TypeName'].unique())

st.markdown('<label class="selectbox-label">Ram (in GB)</label>', unsafe_allow_html=True)
ram = st.selectbox('', [2, 4, 6, 8, 12, 16, 24, 32, 64])

st.markdown('<label class="selectbox-label">Weight of the Laptop</label>', unsafe_allow_html=True)
weight = st.number_input('', step=0.1)

st.markdown('<label class="selectbox-label">Touchscreen</label>', unsafe_allow_html=True)
touchscreen = st.selectbox('', ['No', 'Yes'])

st.markdown('<label class="selectbox-label">IPS</label>', unsafe_allow_html=True)
ips = st.selectbox('', ['No', 'Yes'])

st.markdown('<label class="selectbox-label">Screen Size</label>', unsafe_allow_html=True)
screen_size = st.number_input('', step=0.1)

st.markdown('<label class="selectbox-label">Screen Resolution</label>', unsafe_allow_html=True)
resolution = st.selectbox('', ['1920x1080', '1366x768', '1600x900', '3840x2160', '3200x1800', '2880x1800', '2560x1600', '2560x1440', '2304x1440'])

st.markdown('<label class="selectbox-label">CPU</label>', unsafe_allow_html=True)
cpu = st.selectbox('', df['Cpu Brand'].unique())

st.markdown('<label class="selectbox-label">HDD (in GB)</label>', unsafe_allow_html=True)
hdd = st.selectbox('', [0, 128, 256, 512, 1024, 2048])

st.markdown('<label class="selectbox-label">SSD (in GB)</label>', unsafe_allow_html=True)
ssd = st.selectbox('', [0, 8, 128, 256, 512, 1024])

st.markdown('<label class="selectbox-label">GPU</label>', unsafe_allow_html=True)
gpu = st.selectbox('', df['Gpu brand'].unique())

st.markdown('<label class="selectbox-label">OS</label>', unsafe_allow_html=True)
os = st.selectbox('', df['os'].unique())
...
if st.button('Predict Price'):
    ...
    st.markdown('<h2 class="stSubheader">Prediction</h2>', unsafe_allow_html=True)
    st.markdown(f'<p class="stMarkdown">The predicted price of this configuration is {int(np.exp(pipe.predict(query)[0]))}.</p>', unsafe_allow_html=True)
...
# About section
st.markdown("""
<h2 class="stSubheader">About</h2>
<p class="stMarkdown">
LapValue Predictor is a robust Python-based system designed to predict laptop prices through analysis of provided laptop specifications. Developed by [Your Name], this application utilizes data analytics techniques and the XGBoost algorithm, along with libraries such as scikit-learn, pandas, numpy, matplotlib, and Streamlit for interactive web applications. It delivers high accuracy on test data using machine learning algorithms to predict prices based on users’ specifications, providing them with statistical insights into market values.
</p>
""", unsafe_allow_html=True)
